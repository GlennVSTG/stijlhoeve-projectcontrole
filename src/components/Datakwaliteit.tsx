    ...
    <div style={{ padding: '28px var(--page-pad)', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {state.enrichedOrders.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px 20px', color: 'var(--text-muted)' }}>
          Geen data geladen — importeer eerst Excel-exports
        </div>
      ) : (
              ...
              <div key={severity} style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    borderBottom: isOpen && issues.length > 0 ? '1px solid var(--border-subtle)' : 'none',
                  }}
                ...
                >
                  ...
                  <span style={{
                    ...
                    color: issues.length > 0 ? 'var(--text-secondary)' : 'var(--text-muted)',
                    marginRight: 8,
                  }}>
                  ...
                  issues.length === 0 ? (
                    ...
                    <div>
                      {issues.map((issue, i) => (
                        <div
                          key={i}
                          onClick={() => issue.ordernummer !== '—' && onSelectOrder(issue.ordernummer)}
                          style={{
                            padding: '12px 18px', borderBottom: '1px solid var(--border-subtle)',
                            display: 'flex', alignItems: 'flex-start', gap: 14,
                            cursor: issue.ordernummer !== '—' ? 'pointer' : 'default',
                            transition: 'background 0.1s ease',
                          }}
                          onMouseEnter={ev => { if (issue.ordernummer !== '—') ev.currentTarget.style.background = 'var(--bg-card-hover)' }}
                          onMouseLeave={ev => (ev.currentTarget.style.background = '')}
                        >
